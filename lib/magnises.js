...
MagnisesEvents.prototype.weight = function (events) {
    ...
    //if user is not a plus member, filter out exclusive events
    if (!_this.user.plus_member) {
        events = events.filter(function (event) {
            return event.plus_type.toLowerCase() !== 'plus exclusive';
        });
    }
...
MagnisesEvents.prototype._mapWeight = function (event) {
    ...
    if (event.plus_type.toLowerCase() !== 'none') {
        event.weight += 2;
    ...
    if (event.plus_type.toLowerCase() === 'plus exclusive') {
        event.weight += 1;
    }
